...
class Student:
    def __init__(self,id, config =STD_CFG ):
        ...
        q1, q2 = ss.t.ppf([0.20, 0.95],config["ability_df"])
        self._ability = rng.standard_t(config["ability_df"])
        self.second_quiz = np.round((np.clip(rng.normal(loc  =config["quiz_mean"],scale=  config["quiz_sd"]) + 0.25 * self._ability, 0, a_max=10).item()))
        if self._ability < q1 or self._ability > q2:
            self.prep_time = 0
        else:
            self.prep_time = (rng.lognormal(mean= config["prep_mean"], sigma = config["prep_sigma"]) - self.second_quiz * 0.3 - self._ability * 2.3)
        self.final_grade = np.round(np.clip(rng.normal(loc  =config["final_mean"],scale =  config["final_sd"]) + 0.198 * self.second_quiz + self.prep_time* 0.035, 0,30).item())
